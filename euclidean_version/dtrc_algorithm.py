                    ...
                    for j in range(i + 1, max_search):
                        ...
                        if weight <= DRONE_CAPACITY:
                            if is_virtual_takeoff:
                                ...
                                D = euclidean_distance(from_lat, from_lon, lat_coords[candidate_node], lon_coords[candidate_node])
                            else:
                                    ...
                                    for idx in range(len(route) - 1):
                                        ...
                                        if isinstance(from_node, str) and from_node.startswith('V('):
                                            ...
                                            if isinstance(to_node, str) and to_node.startswith('V('):
                                                ...
                                                segment_distance = euclidean_distance(from_lat, from_lon, to_lat, to_lon)
                                            else:
                                                segment_distance = euclidean_distance(from_lat, from_lon, lat_coords[to_node], lon_coords[to_node])
                                        elif isinstance(to_node, str) and to_node.startswith('V('):
                                            coords = to_node.strip('V()').split(',')
                                            to_lat, to_lon = float(coords[0]), float(coords[1])
                                            segment_distance = euclidean_distance(lat_coords[from_node], lon_coords[from_node], to_lat, to_lon)
                                        else:
                                            segment_distance = euclidean_distance(lat_coords[from_node], lon_coords[from_node], 
